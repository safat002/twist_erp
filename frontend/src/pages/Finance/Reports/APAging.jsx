...
import { getAPAging } from '../../../services/finance';
...
const APAging = () => {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const { data } = await getAPAging({ as_of: asOf.format('YYYY-MM-DD') });
      setSummary(data?.summary || {});
      setBuckets(data?.buckets || {});
    } catch (err) {
      message.error('Unable to load AP aging');
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderBucket = (key) => (
    <Table rowKey={(r) => r.id} loading={loading} dataSource={Array.isArray(buckets[key]) ? buckets[key] : []} columns={columns} pagination={{ pageSize: 10 }} />
  );

  return (
    <div>
      <Space style={{ width: '100%', justifyContent: 'space-between', marginBottom: 16 }}>
        <span style={{ fontSize: 18, fontWeight: 600 }}>AP Aging</span>
        <Space>
