# Fix SalesOrderLine to reference sales.Product instead of inventory.Item
from django.db import migrations, models
import django.db.models.deletion


def migrate_salesorderline_to_sales_product(apps, schema_editor):
    """
    Update SalesOrderLine to reference sales.Product instead of inventory.Item.
    ...
    print("\n" + "="*60)
    print("Migrating SalesOrderLine references from Item to Product")
    print("="*60)

    lines_updated = 0
    lines_skipped = 0

    for line in SalesOrderLine.objects.select_related('product').all():
        # Current product field points to inventory.Item
        item = line.product

        # Find the corresponding sales.Product via legacy_product_id
        # The Item has legacy_product_id, and sales.Product should have the same legacy_product_id
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ...
        ("sales", "0006_alter_salesorderline_product_to_item"),
    ]

    operations = [
        # Step 1: Run data migration to map Item IDs to sales.Product IDs
        migrations.RunPython(
            migrate_salesorderline_to_sales_product,
            reverse_migration
        ),
