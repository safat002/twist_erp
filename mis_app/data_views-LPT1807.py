...
def infer_sql_type(column_data):
    """
    Analyzes a full pandas Series to infer the best SQL data type.
    """
    if column_data.empty:
        return 'VARCHAR(255)'

    # 1. Check for numeric types FIRST, before converting to string
    numeric_series = pd.to_numeric(column_data, errors='coerce')
    if numeric_series.notna().sum() / len(column_data) > 0.9:
        if (numeric_series.dropna() % 1 != 0).any():
            return 'FLOAT'
        else:
            return 'BIGINT'

    # 2. Check for datetime types
    try:
        datetime_series = pd.to_datetime(column_data, errors='coerce')
        if datetime_series.notna().sum() / len(column_data) > 0.9:
            return 'DATETIME'
    except Exception:
        pass 

    # 3. Default to string, calculating length from the ENTIRE column
    max_len = column_data.astype(str).str.len().max()
    if max_len > 512:
        return 'TEXT'
    else:
...
def drop_table(request):
    ...
    try:
        ...
        logger.error(f"[{request.user.username}] Error dropping table '{table_name}' on connection {connection_id}: {str(e)}")
...
def get_table_data(request, connection_id, table_name):
    ...
    try:
        ...
        with engine.connect() as connection:
            q = get_quote_char(engine)
            query = text(f'SELECT * FROM {q}{table_name}{q} LIMIT 1000')
            ...
            stats_query = text(f'SELECT COUNT(*) as row_count FROM {q}{table_name}{q}')
...
def add_column(request):
    ...
    try:
        ...
        q = get_quote_char(engine)
        sql_query = text(f'ALTER TABLE {q}{table_name}{q} ADD COLUMN {q}{column_name}{q} {column_type}')
...
def preview_upload_for_matching(request):
    ...
    try:
        ...
        if temp_filename.lower().endswith('.csv'):
            ...
            df = pd.read_excel(temp_filepath, sheet_name=sheet_name)
