...
def get_external_engine(connection_id, user):
    ...
    try:
        ...
        # --- (PostgreSQL and MySQL logic remains the same) ---
        if connection.db_type == 'postgresql':
            connection_string = f"postgresql+psycopg2://{connection.username}:{connection.password}@{connection.host}:{connection.port}/{connection.db_name}"
            return create_engine(connection_string, connect_args={'connect_timeout': 5}, pool_recycle=1800)
        
        elif connection.db_type == 'mysql':
            connection_string = f"mysql+pymysql://{connection.username}:{connection.password}@{connection.host}:{connection.port}/{connection.db_name}"
            return create_engine(connection_string, connect_args={'connect_timeout': 5}, pool_recycle=1800)

        elif connection.db_type == 'sqlite':
            # --- New Defensive SQLite Path Handling ---
            filepath = connection.filepath
            db_name = connection.db_name
            final_path = None

            # 1. If filepath is provided, use it directly.
            if filepath and filepath.strip():
                final_path = filepath.strip()
            # 2. If no filepath, but db_name is provided, construct a safe path.
            elif db_name and db_name.strip():
                final_path = os.path.join(settings.BASE_DIR, f'{db_name.strip()}.db')
            # 3. As a last resort, use a generic default name.
            else:
            ...
            if db_directory: # Only run makedirs if a directory path is part of final_path
