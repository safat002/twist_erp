...
class User(AbstractUser):
    ...
    default_database = models.CharField(max_length=50, default='default', help_text='Default database connection')
    ...
    def can_upload_data(self):
        ...
        return self.user_type in ['Admin', 'Moderator', 'Uploader']
    
    def can_modify_schema(self):
        """Check if user can modify database schema"""
        return self.user_type in ['Admin', 'Moderator']
    
    def can_save_data_model(self):
        """Check if user can save data model changes"""
        return self.user_type in ['Admin', 'Moderator']
...
class CanvasLayout(models.Model):
    """Store canvas layouts for dashboard design"""
    
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    dashboard = models.OneToOneField(Dashboard, on_delete=models.CASCADE, related_name='canvas_layout')
    ...
    def __str__(self):
        return f"Layout for {self.dashboard.title}"


class DashboardDataContext(models.Model):
    """Store data context for dashboard widgets"""
    
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    dashboard = models.OneToOneField(Dashboard, on_delete=models.CASCADE, related_name='data_context')
    ...
    def __str__(self):
        return f"Data context for {self.dashboard.title}"