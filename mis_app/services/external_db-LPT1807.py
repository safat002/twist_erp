...
class ExternalDBService:
    ...
    def disconnect(self):
        ...
        if self.connection_id in _connection_engines:
            try:
                _connection_engines[self.connection_id].dispose()
    ...
    def get_visible_tables(self) -> List[str]:
        ...
        cache_key = f"tables_{self.connection_id}"
        ...
        try:
            ...
            hidden_tables = []
            if self.connection and self.connection.hidden_tables:
                hidden_tables = [
                    t.strip() for t in self.connection.hidden_tables.split(',') 
                    if t.strip()
                ]
