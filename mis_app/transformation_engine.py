...
def _interval_ops(interval):
    return {
        '[]': (lambda x,lo,hi: x >= lo and x <= hi),
        '[)': (lambda x,lo,hi: x >= lo and x <  hi),
        '(]': (lambda x,lo,hi: x >  lo and x <= hi),
        '()': (lambda x,lo,hi: x >  lo and x <  hi),
    }.get(interval or '[]')

def _build_edges_from_params(values, params):
    mode = (params or {}).get('mode','auto')
    bins = int((params or {}).get('bins') or 10)
    from_v = params.get('from')
    to_v   = params.get('to')
    step   = params.get('step')
    edges_str = params.get('edges')

    if mode == 'custom_edges' and edges_str:
        edges = sorted(set([float(x.strip()) for x in str(edges_str).split(',') if x.strip()!='']))
        return edges

    if mode == 'step' and step:
        lo = float(from_v) if from_v is not None else (min(values) if values else 0.0)
        hi = float(to_v)   if to_v   is not None else (max(values) if values else lo + step*bins)
        edges = []
        x = lo
        while x < hi:
            edges.append(x)
    ...
    # equal width (auto)
    lo = float(from_v) if from_v is not None else (min(values) if values else 0.0)
    hi = float(to_v)   if to_v   is not None else (max(values) if values else lo + bins)
    width = (hi - lo) / max(bins,1)
    edges = [lo + i*width for i in range(bins)]
    edges.append(hi)
    return edges

def _label_for_bin(lo, hi, idx, label_tpl, interval, kfmt=False):
    def fmt(v):
        if not kfmt: return int(v) if float(v).is_integer() else float(v)
        if abs(v) >= 1e6: return f"{round(v/1e6,1)}M".rstrip('0').rstrip('.')
        if abs(v) >= 1e3: return f"{round(v/1e3,1)}K".rstrip('0').rstrip('.')
        return str(int(v) if float(v).is_integer() else v)

    label = label_tpl.replace('{lo}', str(fmt(lo))).replace('{hi}', str(fmt(hi))).replace('{n}', str(idx))
    if label_tpl == '{lo}-{hi}':
        br = {'[]':'[', '[)':'[', '(]':'(', '()':'('}[interval]
        er = {'[]':']', '[)':')', '(]':']', '()':')'}[interval]
        label = f"{br}{fmt(lo)}, {fmt(hi)}{er}"
    return label

def bin_group_rows(rows, groups):
    """Fallback: perform bin grouping after fetching raw rows (list[dict])."""
    # For simplicity, handle single numeric bin group; extend as needed
    bin_group = next((g for g in groups if g.get('method') == 'bin'), None)
    if not bin_group:
        return rows

    col = bin_group['column']
    params = bin_group.get('params') or {}
    interval = params.get('interval','[]')
    include = _interval_ops(interval)
    label_tpl = params.get('label_template','{lo}-{hi}')
    kfmt = bool(params.get('k_format'))

    values = [r.get(col) for r in rows if r.get(col) is not None]
    edges = _build_edges_from_params(values, params)
    ...
    for i in range(len(edges)-1):
        ...
        buckets.append((lo,hi,_label_for_bin(lo,hi,i+1,label_tpl,interval,kfmt)))

    # Assign labels then aggregate the rest of measures as your code does
    out = {}
    for r in rows:
        v = r.get(col)
        label = 'Other'
        ...
        if key not in out:
            out[key] = {**r, col: label}
            # reset non-group measures here if needed
        # accumulate measures (sum/count) — plug into your existing aggregation routine
        # Example: out[key]['amount_sum'] = out[key].get('amount_sum',0) + (r.get('amount') or 0)
