...
class DatabaseConnectionForm(forms.ModelForm):
    ...
    nickname = forms.CharField(
        ...
        help_text='Enter a friendly name for this database connection.'
    )
    ...
    db_type = forms.ChoiceField(
        ...
        help_text='Select the type of database you want to connect to.'
    )
    ...
    db_name = forms.CharField(
        ...
        help_text='Name of the specific database to connect to.'
    )
    ...
    schema = forms.CharField(
        ...
        help_text='Database schema name (if applicable).'
    )
    ...
    filepath = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': '/path/to/database.db or cloud storage URL'
        }),
        help_text='File path for SQLite databases or cloud storage URL.'
    )
    ...
    is_default = forms.BooleanField(
        ...
        help_text='Set as default database for reports and dashboards.'
    )
    ...
    def clean_nickname(self):
        ...
        if nickname and self.request_user:
            queryset = ExternalConnection.objects.filter(owner=self.request_user, nickname__iexact=nickname)
            if self.instance and self.instance.pk:
                queryset = queryset.exclude(pk=self.instance.pk)
            if queryset.exists():
                raise ValidationError("You already have a connection with this name. Please choose a different name.")
        return nickname
    ...
    def clean(self):
        ...
        if db_type == 'sqlite':
            if not filepath:
                self.add_error('filepath', 'File path is required for SQLite databases.')
        else:
            if not host:
